function [G,Gp,ABC]=needle_motion_model2(G, Gp, ABC,nSteps)
...
% %intiially, they are all equal, and are straight lines pointing in the z
% %direction
...
% for iStep=1:nSteps
    ...
    %first, find a new 0heading for the needletip
    mu = 0; %mean of the normal distribution
    sigma = 0.1; %std deviation
        deflection = mu + sigma * randn(1,3); %deviation of the new tangent from the old
    %deflection = [.5 0 0];
    ...
    for nspline = nControlPoints-1:-1:1
        ...
        %get the tangents using klm parameterization
        k1 = X1(1);
        l1 = X1(2);
        m1 = X1(3);
        k2 = X2(1);
...
%         yp = t1(2);
%         zp = t1(3);
%         t2 = [(xp + k1 - k2)/(1-2*lambda) (yp + l1 - l2)/(1-2*lambda) (zp + m1 - m2)/(1-2*lambda)]; 
...
%     grid on
%     axis([0,nControlPoints+nSteps*mu_delta+.5,-1,1,-1,1]);
%     title(sprintf('step %i, total insertion = %f',nSteps,totalDelta));
%     pause(.05);
% end
